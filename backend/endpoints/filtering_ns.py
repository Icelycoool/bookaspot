...
from models import Amenity, Category, Booking, Review, Media
from datetime import datetime
from exts import db
from sqlalchemy import func


search_ns = Namespace("Search", description="Searches for amenities using categories, location, and booking status")
        ...
        if booking_date:
            try:
                ...
                query = query.join(Amenity.bookings).filter(
                    ~((Booking.start_time <= booking_date) & (Booking.end_time >= booking_date))
                )
            except ValueError:
                return {"error": "Invalid date format. Use YYYY-MM-DD."}, 400

        query = query.join(Amenity.category).outerjoin(Amenity.reviews).outerjoin(Amenity.images)
